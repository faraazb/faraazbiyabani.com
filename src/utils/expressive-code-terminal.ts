  ...
  return definePlugin({
    ...
    hooks: {
      ...
      preprocessCode: (context) => {
        const promptOption =
          context.codeBlock.metaOptions.getString(PROMPT_META_KEY);
        const outputRange =
          context.codeBlock.metaOptions.getRange?.(OUTPUT_META_KEY);
      ...
      postprocessRenderedBlock: ({ codeBlock, renderData }) => {
        ...
        if (frameNode.type === "element" && frameNode.tagName === "figure") {
          ...
          frameNode.properties.className = Array.from(
            new Set([
              // @ts-expect-error TODO need to check
              ...(frameNode.properties.className ?? []),
              "has-terminal",
            ]),
          );
        }
        ...
        if (copyButtonWrapper) {
          ...
          if (copyButton?.properties) {
            ...
            copyButton.properties["data-code"] =
              blockData.commandLinesText.join("\n");
          }
        }
      },
    },
